...
public class GameField {
    ...
    public boolean setSymbol(PlayerSymbol symbol, String coordinates) {
        ...
        char valueX = coordinates.charAt(0);
        int row = (valueX - '0') - 1;
        char valueY = coordinates.charAt(2);
        int column = (valueY - '0') - 1;
        ...
        field[row][column] = symbol.getValue();
        return true;
    }
    ...
    boolean isFieldFull() {
        ...
        for (int column = 0; column < field.length; column++) {
            for (int row = 0; row < field[column].length; row++) {
                if (field[column][row] == EMPTY_CELL) {
                    return false;
                }
            }
        }
...
//        Пройтись по всем строкам массива и вернуть true, если хотя бы одна из них полностью заполнена символами игрока.
    ...
    boolean checkDiagonals(char symbol) {
        ...
        return (field[0][2] == symbol) && (field[1][1] == symbol) && (field[2][0] == symbol);
    }
}
